...
if not BOT_TOKEN:
    ...
    print("API_BASE=your_api_base_url (optional)")
    exit(1)

if not API_BASE:
    print("⚠️ WARNING: API_BASE not found in .env file!")
    print("Using default API base URL")
...
async def search_number_api(number):
    ...
    try:  
        ...
        if response.status_code != 200:  
            return "🛡️ Dark Shadow INFORMATION 🛡️\n\n" \
                   "🎯 TARGET: {}\n\n" \
                   "❌ DATABASE ERROR\n\n" \
...
def format_cybersecurity_report(user_data, number, record_count, current_time):
    ...
    report += "🎯 Dark Shadow INFORMATION\n"
    ...
    report += f"├─ 🛡️ Anish Exploits\n"
    report += f"├─ 👨‍💻 Developer: @Cyb3rS0ldier\n"
...
def main():
    ...
    print(f"🤖 BOT_TOKEN: {'✓ Loaded' if BOT_TOKEN else '✗ Not found'}")
    print(f"🌐 API_BASE: {API_BASE[:30]}...")
